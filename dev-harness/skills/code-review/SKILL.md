...
- **`--json`을 주는 이유**: 기본 출력은 렌더된 마크다운이라 **발견별 confidence가 통째로 빠진다** — policy §확신도의 "≥0.8만 채택"이 실행 불가가 된다(2026-08-10까지 이 필터는 한 번도 작동하지 않았다). `--json`이면 스키마 검증된 payload가 그대로 나오고 `findings[]`에 `confidence`가 들어 있다(`schemas/review-output.schema.json`에서 **required**). `--cwd`와 같은 부류로 파서(`handleReviewCommand`의 `booleanOptions`)에는 있고 usage 문자열에만 누락돼 있다.
