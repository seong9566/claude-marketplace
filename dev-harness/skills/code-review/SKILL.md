...
- **이 Bash 호출은 `run_in_background: true`로 띄운다.** Bash 도구 기본 타임아웃은 2분인데 리뷰는 그보다 오래 걸리고, 타임아웃이 나면 **codex 워커가 함께 죽어** 리뷰가 통째로 날아간다. 백그라운드로 띄우면 하네스가 종료를 알려준다. companion `--wait`의 자체 타임아웃(기본 4분)과는 별개 문제다 — `--wait`를 줄여도 Bash 쪽 2분이 먼저 끊는다. **결과 파일은 위 `$out`처럼 `mktemp`로 호출마다 격리한다** — 백그라운드가 기본이면 고정 경로가 동시 실행·재시도끼리 서로를 덮어쓴다(아래 "동시 리뷰 금지"는 문서 규칙일 뿐 강제되지 않는다). PR 번호·HEAD를 넣는 것만으로는 부족하다 — 같은 PR·같은 커밋을 재시도하면 그대로 겹친다.
