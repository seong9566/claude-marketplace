...
**Check the repo's CLAUDE.md before you pick one.** If it names the command to use — or rules one out — that wins over your own detection. A harness-native tool the repo has excluded (because it forces a branch name, a base ref, or a directory the repo cannot accept) is the wrong mechanism there, not the preferred one.
