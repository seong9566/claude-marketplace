#!/bin/bash
# docs/ARCHITECTURE.md 빠른 grep 점검(9종)의 단일 출처 — Stop hook + 수동 리포트 겸용
...
check_1_1() { # core/shared → app·feature import 금지
  check_import_targets '^lib/(app|features)(/|$)' "" lib/core lib/shared
  return 0
}

check_1_2() { # feature 간 직접 import 금지
  local f fname
  for f in lib/features/*/; do
    [ -d "$f" ] || continue
    fname=$(basename "$f")
    check_import_targets '^lib/features/[^/]+(/|$)' "lib/features/$fname" "$f"
  done
  return 0
}

check_1_3() { # domain/shared → Flutter import 금지(shared/widgets 제외)
  find lib/features/*/domain lib/shared -type f -name "*.dart" \
...
check_1_4() { # presentation UI·VM → data import 금지(providers 제외)
  check_import_targets '^lib/features/[^/]+/data(/|$)' "" \
    lib/features/*/presentation/screens \
    lib/features/*/presentation/widgets \
    lib/features/*/presentation/view_models
  return 0
}

check_1_5() { # domain/shared → data import 금지
  check_import_targets '^lib/features/[^/]+/data(/|$)' "" \
    lib/features/*/domain lib/shared
  return 0
}
...
check_3_2() { # Repository 인터페이스/구현 배치
  grep -rn "class .*RepositoryImpl" lib/features/*/domain --include="*.dart" 2>/dev/null
  grep -rn "abstract .*Repository" lib/features/*/data --include="*.dart" 2>/dev/null
  return 0
}
...
item_title() {
  case "$1" in
    1_1) echo "core/shared → app·feature import 금지" ;;
    1_2) echo "feature 간 직접 import 금지" ;;
...
item_fix() {
  case "$1" in
    1_1) echo "공유 대상이면 core/shared 인프라로 끌어올리고, 아니면 feature 내부로 되돌린다. core/shared 함수가 상위 레이어 타입을 받지 않게 한다." ;;
    ...
    1_4) echo "UI·ViewModel은 domain의 UseCase·Entity·Repository 인터페이스만 참조한다. 구현 배선은 presentation/providers로 옮긴다." ;;
