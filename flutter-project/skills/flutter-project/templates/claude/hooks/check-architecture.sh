...
check_1_1() { # core/shared → app·feature import 금지
  grep -rnE "^[[:space:]]*import[[:space:]].*(app|features)/" \
...
check_1_2() { # feature 간 직접 import 금지
  ...
  for f in lib/features/*/; do
    ...
    find "$f" -type f -name "*.dart" -exec awk -v current="$fname" -v boundary="features" '
...
check_1_5() { # domain/shared → data import 금지
  grep -rnE "^[[:space:]]*import[[:space:]].*data/" \
    lib/features/*/domain lib/shared --include="*.dart" 2>/dev/null
  return 0
}
